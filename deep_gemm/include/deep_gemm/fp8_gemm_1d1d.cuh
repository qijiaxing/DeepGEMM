...
fp8_gemm_kernel(__nv_bfloat16* gmem_d,
                ...
                const __grid_constant__ CUtensorMap tensor_map_scales_b,   // scale b
                const __grid_constant__ CUtensorMap tensor_map_d) {
#if (defined(__CUDA_ARCH__) and (__CUDA_ARCH__ >= 900)) or defined(__CLION_IDE__)
    // Scaling checks
    DG_STATIC_ASSERT(BLOCK_K == 128, "Only support per-128-channel FP8 scaling");
    DG_STATIC_ASSERT(ceil_div(BLOCK_N, BLOCK_K) == 1, "Too much B scales in a single block");   // TODO: needed for 1d1d?
    ...
    // Configs
    // shape k = num_iters * num_stages * block_k
    ...
    if (threadIdx.x == kNumMathThreads) {
        ...
        cute::prefetch_tma_descriptor(reinterpret_cast<cute::TmaDescriptor const*>(&tensor_map_scales_b));   // scale b
    ...
    float* smem_scales_b[kNumStages];   // scale b
    ...
    // SMEM layout: [D, numstages * A, num_stages * B, num_stages * scaleA1, num_stages * scaleB1]
    #pragma unroll
    ...
    constexpr int kNumTMARegisters = 40;   // TODO: why 40?
...
public:
    ...
    static void run(__nv_bfloat16* gmem_d, int* grouped_layout,
        ...
        auto status = cudaLaunchKernelEx(&config, kernel,
                                         ...
                                         tma_scales_a_desc, tma_scales_b_desc, tma_d_desc);
        DG_HOST_ASSERT(status == cudaSuccess);
    }

    /* Tensor A, shape [M*G, K], tile [BLOCK_M, BLOCK_K] */
    template <typename T>
    static CUtensorMap make_2d_tma_a_desc(T* global_address, uint32_t shape_m) {
        return make_2d_tma_desc(global_address, Layout::RowMajor,
                                shape_m * (kGemmType == GemmType::GroupedMasked ? kNumGroups : 1), SHAPE_K,
                                BLOCK_M, BLOCK_K);
    }

    /* Tensor B, shape [K, N*G], tile [BLOCK_K, BLOCK_N] */
    template <typename T>
    static CUtensorMap make_2d_tma_b_desc(T* global_address) {
        return make_2d_tma_desc(global_address, Layout::ColMajor,
                                SHAPE_K, SHAPE_N * (kGemmType != GemmType::Normal ? kNumGroups : 1),
                                BLOCK_K, BLOCK_N);
    }

    /* Tensor d, shape [M*G, N], tile [BLOCK_M, BLOCK_N] */
    template <typename T>
    ...
    /* Tensor A, shape [M*G, K], tile [BLOCK_M, BLOCK_K] */
    /* Scale a, shape [M, K/BLOCK_K*G], tile [BLOCK_M, 1] */
    template <typename T>
    static CUtensorMap make_2d_tma_scales_a_desc(T* global_address, uint32_t shape_m) {
        ...
        return make_2d_tma_desc(global_address, Layout::ColMajor,
                                shape_m, ceil_div(SHAPE_K, BLOCK_K) * (kGemmType == GemmType::GroupedMasked ? kNumGroups : 1),
                                BLOCK_M, 1,
                                CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_NONE);
    }

    /* Scale b, shape [N, K/BLOCK_K*G], tile [BLOCK_N, 1] */
    template <typename T>
